...
class GPCoregionalizedRegressionWarped(GP):  # pylint:disable=too-many-ancestors
    """
    Gaussian Process model for heteroscedastic multioutput regression
    This is a thin wrapper around the models.GP class, with a set of sensible defaults
    :param X_list: list of input observations corresponding to each output
    ...
    def __init__(  # pylint:disable=too-many-arguments
        ...
        super(
            GPCoregionalizedRegressionWarped, self
    ...
    def build_XY(input_list, output_list=None, index=None):
        ...
        if warping_function is None:
            self.warping_function = KumarWarping(
                self.X_untransformed, warping_indices, epsilon, Xmin, Xmax
            )
        else:
